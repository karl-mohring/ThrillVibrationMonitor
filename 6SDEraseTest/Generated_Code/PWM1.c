...
**     Date/Time   : 2013-06-06, 17:07, # CodeGen: 39
**     Abstract    :
...
**             Xtal ticks              : 200        96
**             microseconds            : 17         8
**             seconds (real)          : 0.000016625 0.000008
**
**     Contents    :
...
static void SetRatio(void)
{
  if (ActualRatio.Value == 0xFFFFU) {  /* Duty = 100%? */
    ...
    uint16_t Result;
    Result = 0U; /* HI * HI */
    Tmp1.Value = (uint16_t)((uint16_t)ActualRatio.BB.Hi * 0x85U); /* HI * LO */
    Result += Tmp1.BB.Hi;
    Tmp2.BB.Lo = 0U;
    if ((Tmp2.BB.Lo += Tmp1.BB.Lo) < Tmp1.BB.Lo) {
      ++Result;                        /* carry to result */
    }
    Tmp1.Value = (uint16_t)((uint16_t)ActualRatio.BB.Lo * 0x85U); /* LO * LO */
...
byte PWM1_SetDutyUS(word Time)
{
  ...
  if (Time > 0x11U) {                  /* Is the given value out of range? */
    return ERR_RANGE;                  /* If yes then error */
  }
  PE_Timer_LngMul((dword)Time, 0x0F6603DAUL, &rtval); /* Multiply given value and High speed CPU mode coefficient */
...
void PWM1_Init(void)
{
  ...
  ActualRatio.Value = 0x7B31U;         /* Store initial value of the ratio */
  /* TPM2MOD: BIT15=0,BIT14=0,BIT13=0,BIT12=0,BIT11=0,BIT10=0,BIT9=0,BIT8=0,BIT7=1,BIT6=0,BIT5=0,BIT4=0,BIT3=0,BIT2=1,BIT1=0,BIT0=0 */
  setReg16(TPM2MOD, 0x84U);            /* Set modulo register */ 
