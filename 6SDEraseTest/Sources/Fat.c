...
UINT16 FAT_SearchAvailableFAT(UINT16 u16CurrentFAT) {
	...
	while (u16Sector
			< (((u16FAT_Root_BASE - u16FAT_FAT_BASE) >> 1) + u16Main_Offset)) {
 ...
 *     *** FAT_DataSectorWrite ***
 *
 *     Description: Write a 512 byte data sector.
 *                  Assume:
 *                         - SD Card has been previously erased.
 *                         - Write a full sector at a time.
 *
 *     Entry: pu8DataPointer is a pointer to the 512 byte buffer to be written. 
 *
 ****************************************************************************************/
void FAT_DataSectorWrite(UINT8 *pu8DataPointer) {
//    UINT32 u32SectorToWrite;

//    /* Calculate the sector to be written */
//    u32SectorToWrite= u16FAT_Data_BASE + WHandler.ClusterIndex + (WHandler.CurrentFatEntry-2)*u16FAT_Cluster_Size;

	/* Write the sector */
	StorePhysicalBLock(u32DataSector++, pu8DataPointer);
	// Write Buffer to Sector

//    WHandler.ClusterIndex++;          // increment the current sector number in the cluster
//    WHandler.File_Size += 512;        // update the file size, 512 bytes per sector

//    /* If the cluster is now full update the FAT table */
...
void FAT_FileWrite(UINT8 *pu8DataPointer, UINT32 u32Size) {
	...
	while (u32Size) {
		...
		StorePhysicalBLock(u32SectorToWrite, ag8FATWriteBuffer);
		// Write Buffer to Sector
...
UINT16 FAT_FileRead(UINT8 *pu8UserBuffer) {
	...
	u32SectorToRead = u16FAT_Data_BASE
			+ ((RHandler.FAT_Entry - 2) * u16FAT_Cluster_Size)
			...
			if (u8Function == READ || u8Function == MODIFY) {
				...
				if (sFileStructure->FileName[0] == u8FileName[0]) {
					...
					while (u8Flag == True && u8Counter < 10) {
						u8Counter++;
						if (sFileStructure->FileName[u8Counter]
								!= u8FileName[u8Counter])
							u8Flag = False;
					}
					if (u8Flag == True) {
						/* If Read Function */
						if (u8Function == READ) {
							RHandler.Dir_Entry = (u16Block * RootEntrySize)
									+ ((u16Index) / RootEntrySize);
							RHandler.File_Size = LWordSwap(
									sFileStructure->SizeofFile);
							RHandler.FAT_Entry =
									ByteSwap(sFileStructure->ClusterNumber);
							RHandler.SectorOffset = 0;
						...
						/* If Modify Function */
							...
							WHandler.Dir_Entry = (u16Block * RootEntrySize)
									+ ((u16Index) / RootEntrySize);
							WHandler.File_Size = LWordSwap(
									sFileStructure->SizeofFile);
							WHandler.BaseFatEntry =
									ByteSwap(sFileStructure->ClusterNumber);
							...
							if (WHandler.BaseFatEntry != 0) {
								...
								do {
									WHandler.CurrentFatEntry =
											WHandler.BaseFatEntry;
									WHandler.BaseFatEntry = FAT_Entry(
											WHandler.CurrentFatEntry, 0,
											NEXT_ENTRY);
								} while (WHandler.BaseFatEntry != 0xFFFF);
								WHandler.BaseFatEntry = u16Temporal;
							} else {
								WHandler.BaseFatEntry = FAT_SearchAvailableFAT(
										0);
								WHandler.CurrentFatEntry =
										WHandler.BaseFatEntry;
							}
							...
							u16Temporal = (UINT16) WHandler.File_Size
									% (u16FAT_Sector_Size << u8Counter);
							WHandler.ClusterIndex = u16Temporal
									/ u16FAT_Sector_Size;
							WHandler.SectorIndex = u16Temporal
									% u16FAT_Sector_Size;
							u8ErrorCode = FILE_FOUND;
						}
					}
