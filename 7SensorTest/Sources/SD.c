...
//    UINT16 Count16;
    ...
    /* Configure the SD Card inserted detection pin */
    _SD_PRESENT = _IN;          // Configure the data direction as IN.
    SD_PRESENT_PU = 1;          // Enable Internal Pull-UP Resistor
    
    /* Initialize SPI Module */
    InitSPI();                  // SS high, 375kHz clock.
...
//        return(INIT_FAILS);
    ...
    /* Read the Operation Conditions Register OCR & check for correct voltage range.
       ...
       Send READ_OCR CMD58.
       CMD58 response R3 (5bytes - R1 + OCR). */ 
    SPI_SS=ENABLE;
...
//        return(INIT_FAILS);
    ...
    /*  Set the Block Length to 512 butes */
    SPI_SS=ENABLE;
...
//    }
    ...
    while(SPI_Receive_byte()==0x00);                  // Wait while the card is busy
...
UINT8 SD_SendCommand(UINT8 u8SDCommand,UINT8 u8SDResponse){
    ...
    if(u8Counter)   return(OK);               // 0x00
    else            return(COMMAND_FAILS);    // 0x01
...
//  } while((u8Temp & 0xC0) && mlCounter > 0);     // Main loop test
    ...
    /* Set up return condition */
    if(gu8SD_Response[0] == u8SDResponse) return(OK); // 0x00
